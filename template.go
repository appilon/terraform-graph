...
<body>
    ...
    <script id="terraform-data" type="application/json">%s</script>
    <script>
        // Get JSON data
        var treeData = JSON.parse(document.getElementById('terraform-data').innerHTML);
        ...
        function collapseWhen(fn, d) {
            if (d.children) {
                if (fn(d)) {
                    ...
                    d.children.forEach(d => collapseWhen(fn, d))
                }
            }
        }
        collapseWhen(d => d.children.length > 4, root);
    </script>
</body>
